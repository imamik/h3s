...
func TestClientCreateRecordSuccess(t *testing.T) {
	...
	assert.NotNil(t, requestBodyReader, "The request body should not be nil")
	jsonRequestBody, _ := io.ReadAll(requestBodyReader)
	assert.Equal(t, `{"ttl":3600,"zone_id":"wwwlsksjjenm","type":"A","name":"zone1.online","value":"192.168.1.1"}`, string(jsonRequestBody))
}
...
func TestClientUpdateRecordSuccess(t *testing.T) {
	...
	recordWithUpdatesJSON := `{"ttl":3600,"zone_id":"wwwlsksjjenm","id":"12345678","type":"A","name":"zone2.online","value":"192.168.1.1"}`
...
func TestClientHandleUnauthorizedRequest(t *testing.T) {
	...
	require.ErrorContains(t, err, "'Invalid API key'", "errors message didn't contain error message from API.")
}
